...
class VERI(Dataset):
    """
        mode:
            source: 2811 (train) + 2811(test) different identities.
            train:  2811 different identities.
            test:   2811 different identities.
    ...
    def __init__(self, 
        ...
        if self.mode == 'test':
            self.dataset_path = os.path.join(dataset_path, 'gallery_images')
            self.csv_path = os.path.join(csv_path, 'veri_test_list.txt')
            
        elif self.mode == 'query':
            self.dataset_path = os.path.join(dataset_path, 'probe_images')
            self.csv_path = os.path.join(csv_path, 'veri_query_list.txt')
            
        elif self.mode == 'train' or self.mode == 'source':
            self.dataset_path = os.path.join(dataset_path, 'train_images')
...
#         lines = reader.readlines()
        data_f = pd.read_csv(self.csv_path, sep=' ', header=None)        
        if self.mode == 'test' or self.mode == 'query':
            self.image_names = data_f[0]
            self.image_labels = data_f[1]
            self.camera_id = data_f[2]
            
        else:
            self.image_names = data_f[0]
            map_f = dict(zip(sorted(data_f[1].unique()),list(range(0,config.VRIC_CLASS_NUM))))
            data_f['id'] = data_f[1].map(map_f)
            self.image_labels = data_f['id']
            self.camera_id = data_f[2]
